...
async function gradeOpenAnswer(q, answer, reading) {
  const subparts = /\(а[,)]/.test(q.stem || "");
  const system = "Ты — ОЧЕНЬ СТРОГИЙ экзаменатор ВШБ НИУ ВШЭ по стратегическому менеджменту, известный беспощадными оценками. Ты придираешься к деталям и НЕ завышаешь баллы — лучше недооценить, чем переоценить. Студент готовится к реальному экзамену с жёстким преподавателем, поэтому твоя задача — показать правду и заставить улучшить ответ, а не похвалить. Критерии (как на экзамене): (1) корректность применения концепций С ОБЯЗАТЕЛЬНЫМ указанием авторов; (2) логика и структура аргументации; (3) использование КОНКРЕТНЫХ фактов кейса; (4) отсутствие методологических ошибок и подмены концепций. Правила: за каждый нераскрытый пункт рубрики и каждого неназванного автора снижай балл; общие слова без концепций и авторов = почти ноль; называние термина без объяснения МЕХАНИЗМА засчитывается лишь частично; вода и пересказ условия баллов не приносят. Отвечай по-русски, предметно, без воды и без комплиментов.";
  const fmtInstr = subparts
    ? "Оцени КАЖДЫЙ подпункт ОТДЕЛЬНО, каждый из 4 баллов (всего 12). Верни РОВНО в таком формате (markdown):\n**Подпункт (а): X/4** — что засчитано, чего не хватило, что нужно было добавить (концепции, авторы, факты).\n**Подпункт (б): Y/4** — аналогично.\n**Подпункт (в): Z/4** — аналогично.\n**ИТОГ: N/12** — и одной фразой: какая это оценка по 10-балльной шкале и сдал бы или нет.\n**Методологические ошибки:** подмены концепций, неверная атрибуция — если есть.\n**Как поднять балл:** 3–4 конкретных правки (что добавить, какого автора назвать, какой факт кейса использовать).\n**Что дочитать/добить:** 2–3 конкретные концепции/сравнения из темы для повторения."
    : "Оцени СТРОГО и верни РОВНО в таком формате (markdown):\n**Балл: X/12** — и одной фразой, какая это оценка по 10-балльной шкале и сдал бы или нет.\n**Что засчитано:** конкретные пункты рубрики и названные авторы.\n**Что пропущено/слабо:** каких концепций, авторов и фактов кейса не хватает — поимённо.\n**Методологические ошибки:** подмены концепций, неверная атрибуция, общие слова — если есть.\n**Как поднять балл:** 3–4 конкретных правки (что добавить, какого автора назвать, какой факт использовать).\n**Что дочитать/добить:** 2–3 конкретные концепции/сравнения из темы для повторения.";
  const user = `КЕЙС: ${q.case}\nВОПРОС (с подпунктами и баллами): ${q.stem}\n\nЭТАЛОННАЯ РУБРИКА (что ОБЯЗАНО прозвучать по подпунктам):\n${q.must.map((m, i) => (i + 1) + ". " + m).join("\n")}\n\nТИПИЧНЫЕ ОШИБКИ (штрафуй за них): ${q.errors.join("; ")}\n\nКЛЮЧЕВЫЕ КОНЦЕПЦИИ ТЕМЫ ДЛЯ ПОВТОРЕНИЯ: ${reading}\n\nОТВЕТ СТУДЕНТА:\n${answer}\n\n${fmtInstr}`;
